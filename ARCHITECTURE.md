...
Telegram Mini App 发卡商城，采用双应用架构。
...
- 数据库：MySQL
- ORM：Prisma
- Prisma Schema 单一来源：`/workspace/prisma/schema.prisma`
- 部署：`deploy.sh` + PM2
- 当前支付能力：余额支付。EPUSDT / OKPAY 暂未接入，接口层应只允许 `BALANCE`。
...
│   │   ├── telegram-user.ts   # Telegram 用户 upsert 工具
...
项目不使用传统账号密码登录，用户端完全依赖 Telegram WebApp 身份系统。
...
- 用户自动创建：任何需要登录的 API 首次访问时自动注册。
- 无状态：每次请求独立验证 initData 签名，不依赖 session/token。
- 用户端 API 统一身份来源：`x-init-data` 请求头。

| 文件 | 作用 |
| --- | --- |
| `mini-app/lib/telegram.ts` | HMAC 签名验证、用户信息解析 |
| `mini-app/lib/telegram-user.ts` | Telegram 用户 upsert 工具 |
| `mini-app/lib/api-fetch.ts` | 前端自动注入 `x-init-data` 头 |
| `mini-app/app/api/user/route.ts` | GET `/api/user`，返回用户信息和余额 |
...
| 文件 | 作用 |
| --- | --- |
| `mini-app/app/api/products/route.ts` | 商品列表 API |
| `mini-app/app/api/products/[id]/route.ts` | 商品详情 API |
| `mini-app/app/page.tsx` | 首页（商品列表 + 分类筛选） |
| `mini-app/app/product/[id]/page.tsx` | 商品详情页（下单入口） |
...
Body: {
    ...
    paymentMethod: 'BALANCE'
        ...
        │
        ▼
校验：商品存在且上架 → 规格匹配 → 数量合法（每次限 1 种商品）
        │
        ▼
Prisma $transaction（原子操作）:
    ...
    2. 创建订单
       order.create({ orderNo, userId, status:'PENDING', payStatus:'PENDING', paymentMethod:'BALANCE', ... })
...
Prisma $transaction → processBalancePayment(tx, orderId, userId)：
        │
    ── 原子状态锁 ──
    1. order.updateMany({ id, userId, status:'PENDING', payStatus:'PENDING', paymentMethod:'BALANCE' })
...
| 原子锁 | `updateMany + where` 条件作为状态锁（`PENDING → PROCESSING`） |
| 事务 | 所有关键写操作在 Prisma transaction 内，异常自动回滚 |
...
- 首次使用通过 `/api/admin/auth/setup` 初始化。
...
| 商品管理 | 商品 CRUD 接口 | 商品/规格增删改查 |
| 卡密导入 | 卡密导入接口 | 批量导入卡密 |
| 用户充值 | 用户 PATCH 接口 | 手动充值余额，记录 `recharge_logs` 与 `balance_logs` |
...
- `AdminConfig`：TOTP 密钥。
- `SystemSetting`：KV 配置（订单过期时间、未来支付网关参数等）。
- `PaymentCallbackLog`：第三方支付回调日志（预留）。

## 九、核心模型字段说明

| 模型 | 关键字段 | 说明 |
| --- | --- | --- |
| `User` | `tgId` | Telegram 用户 ID，唯一标识 |
| `Order` | `status`, `payStatus`, `expiresAt` | 状态机：`PENDING → PROCESSING → COMPLETED / CANCELLED` |
| `CardSecret` | `content`, `status`, `lockedOrderId` | 卡密内容唯一，状态机：`AVAILABLE → LOCKED → SOLD` |
| `PaymentRecord` | `orderId + channel` | 每笔订单每种支付方式最多一条记录 |
| `DeliveryLog` | `cardSecretId` | 每条卡密最多发一次 |
| `ProductSpec` | `productId` | 商品可选规格（月卡/季卡/年卡等） |

## 十、关键设计原则

1. 数据完整优先：宁可慢，不能丢数据或重复发卡。
2. 幂等优先：所有关键写入（支付、发卡）都要保证幂等。
3. 数据库约束兜底：应用层防不住时，数据库唯一索引作为最后防线。
4. 原子操作防并发：使用 `updateMany + where` 条件模拟状态锁，不用应用层互斥锁。
